...
import { getCommandLineArgs } from '../utils/getCommandLineArgs';
import { isElectronQA } from '../utils/misc';
...
class Logger {
    ...
    private readonly logQueue: LogMsg[];
    ...
    constructor() {
        ...
        if (!isElectronQA) {
            transports.file.file = path.join(this.logPath, `app_swift_search_${Date.now()}.log`);
    ...
    public setLoggerWindow(window: Electron.WebContents): void {
        ...
        if (this.loggerWindow) {
            ...
            if (Object.keys(logMsgs).length) {
                this.loggerWindow.send('logger.ts', logMsgs);
            }
        }
    }
     ...
     */
    private formatLogMsg(level: LogLevel, details: any): LogMsg {
        return {
            details,
            level,
     ...
     * @param logMsg {LogMsg}
     */
    private sendToCloud(logMsg: LogMsg): void {
        ...
        if (this.loggerWindow) {
            this.loggerWindow.send('logger.ts', { msgs: [ logMsg ], logLevel: this.desiredLogLevel, showInConsole: this.showInConsole });
            return;
        }
    ...
    private cleanupOldLogs(): void {
        ...
        files.forEach((file) => {
            if (file === '.DS_Store' || file === 'app.logger.ts') {
                return;
            }
